...
const setupHistoryCommand = (data, iotajs, vorpal) => {
  ...
  const showItems = (number, callback) => {
    ...
    transfers.forEach((bundle, index) => {
      ...
      vorpal.log(`${index < 9 ? ' ' : ''}${index+1}: ${chalk.yellow(shortHash)} - ${type} ${address} - ${leftPad(value, biggestValue.length)} - ${persisted ? chalk.green('confirmed') : chalk.yellow('pending  ')} - ${time}`);
    });
