...
const setupHistoryCommand = (data, iotajs, vorpal) => {
  vorpal
    ...
    .action((args, callback) => {
      ...
      const number = args.n || 10;

      const categorizedTransfers = iotajs.utils.categorizeTransfers(data.accountData.transfers, data.accountData.addresses);

      data.accountData.transfers.reverse().slice(0, number).forEach((bundle, index) => {
